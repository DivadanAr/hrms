...
export default function ModalTambahDepartemen({
  isOpen,
  onClose,
  onSuccess,
  onError,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      handleClose();
      onSuccess("Departemen berhasil ditambahkan.");
    } catch {
      onError("Gagal menambahkan departemen. Coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClose = () => {
    if (loading) return;
    setForm({ nama_departemen: "", keterangan: "" });
    setFieldError("");
    onClose();
  };
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Isi form berikut untuk menambah departemen baru
              </p>
            </div>
          </div>
          <button
