...
export default function ModalDeleteKaryawan({
  isOpen,
  data,
  onClose,
  onSuccess,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onClose();
      onSuccess(`Karyawan "${data?.nama}" berhasil dihapus.`);
    } catch {
      onError("Gagal menghapus karyawan. Coba lagi.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="px-5 py-5 space-y-3">
          <div className="flex items-start gap-3 p-3 rounded-lg bg-red-50 border border-red-100">
            ...
            <p className="text-xs text-red-600 leading-relaxed">
              Tindakan ini tidak dapat dibatalkan. Akun user terkait juga akan
              ikut terhapus.
            </p>
          </div>

          <div className="p-3 rounded-lg bg-gray-50 border border-gray-100 space-y-1.5">
            <p className="text-xs text-gray-400">Karyawan yang akan dihapus:</p>
            <p className="text-sm font-medium text-gray-800">{data.nama}</p>
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-500">NIP: {data.nip}</span>
              {data.id_jabatan && (
